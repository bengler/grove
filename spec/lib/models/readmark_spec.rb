...
describe Readmark do
  ...
  it "gets updated autmatically as posts are added" do
    ...
    r1.reload
    expect(r1.unread_count).to eq 1
    expect(r2.unread_count).to eq 0
    p.paths |= ["a.c.d"]
    p.save!
    r2.reload
    expect(r2.unread_count).to eq 1
    p.paths = []
    p.save!
    r2.reload
    expect(r2.unread_count).to eq 0
  ...
  it "can set a readmark and update readmark count accordingly" do
    ...
    expect(readmark.unread_count).to eq 1
    readmark = Readmark.set!(1, "a.b", posts[-5].id)
    expect(readmark.unread_count).to eq 4
